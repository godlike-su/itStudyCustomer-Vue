			...
			<!-- 头像 昵称 -->
			  <div class="comments-list-item-heading">
				<van-image
				  ...
				  fit="cover"
				  @click="$router.push({name: 'otherHome-homeSort', query: {id: item.createId}})"
				/>
...
export default {
	...
	mounted() {
		...
		req.pageNumber = this.$route.query.pageNumber;
		this.$axios({
			url: '/customer/analysisShowReplyList.do',
			method: 'post',
			data: req
		}).then(res => {
			if(res.data.error==0) {
				for(let i=0; i<res.data.data.analysisReplyList.length; i++){
					res.data.data.analysisReplyList[i].content = this.unemoji(res.data.data.analysisReplyList[i].content)
				}
				this.replyList = res.data.data.analysisReplyList
			}else {
